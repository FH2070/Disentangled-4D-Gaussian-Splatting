...
from nerfstudio.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation_from_rotor8, strip_symmetric, \
    build_scaling_rotation, quaterion2rotor, rotornorm, slice_4d, compute_cov3,build_rotation
...
class SplatfactoModel(Model):
    ...
    # @property
    # def accelera(self):
    #     return self.gauss_params["accelera"]
    ...
    def load_state_dict(self, dict, **kwargs):  # type: ignore
        ...
        if "xyz" in dict:
            ...
            # for p in ["xyz","t", "scales_xyz","scales_t" ,"quats1","quats2","accelera", "features_dc", "features_rest", "opacities"]:
    ...
    def cull_gaussians(self, extra_cull_mask: Optional[torch.Tensor] = None):
        ...
        if self.step < self.config.stop_screen_size_at:
            ...
            toobigs_x = (self.scales[:,0]>0.5).squeeze()
            # toobigs=(self.max_2Dsize > 20).squeeze()
            toobigs_y = (self.scales[:,1]>0.5).squeeze()
            toobigs_z = (self.scales[:,2]>0.5).squeeze()
            # toobigs_ws = (self.scales[:, :3].max(dim=1).values > 0.01 * self.scene_extent).squeeze()
            # toobigs=toobigs|toobigs_ws
            toobigs=toobigs_x|toobigs_y|toobigs_z
            culls = culls | toobigs
    ...
    def split_gaussians(self, split_mask,samps):
        ...
        stds3d = torch.exp(self.scales_xyz[split_mask]).repeat(samps, 1)#self.scales[split_mask, 0:3].repeat(samps, 1)
        means3d = torch.zeros((stds3d.size(0), 3), device=self.xyz.device)
        scaled_samples_xyz = torch.normal(mean=means3d, std=stds3d)
        #quats = self.quats[split_mask] / self.quats[split_mask].norm(dim=-1, keepdim=True)  # normalize them first
        #rots = quat_to_rotmat(quats.repeat(samps, 1))  # how these scales are rotated
        _rotors1, _rotors2 = self.quats1,self.quats2
        rots = build_rotation(_rotors1[split_mask]).repeat(samps, 1, 1)
        rotated_samples = torch.bmm(rots, scaled_samples_xyz.unsqueeze(-1)).squeeze(-1)
        new_xyz = rotated_samples + self.xyzt[split_mask,:3].repeat(samps, 1)
        # new_xyz = self.xyzt[split_mask,:3].repeat(samps, 1)
        
        stds_t = torch.exp(self.scales_t[split_mask]).repeat(samps, 1) #self.scales[split_mask, 3:4].repeat(samps, 1)
        means_t = torch.zeros((stds_t.size(0), 1), device=self.xyz.device)
        scaled_samples_t = torch.normal(mean=means_t)
        new_t = scaled_samples_t + self.xyzt[split_mask,3:4].repeat(samps, 1)
        
        new_xyz = new_xyz
        new_t = new_t#self.xyzt[split_mask,3:4].repeat(samps, 1)
        ...
        new_scale_t = (self.scales[split_mask,3:4] / size_fac).repeat(samps, 1)
        # step 5, sample new quats
        new_quats1 = self.quats1[split_mask].repeat(samps, 1)
        new_quats2 = (self.quats2[split_mask]).repeat(samps, 1)
        # new_accelera = (self.accelera[split_mask]).repeat(samps, 1)
        out = {
            "xyz": new_xyz,
            "t" : new_t,
    ...
    def split_gaussians_t(self, split_mask,samps):
        """
        This function splits gaussians that are too large
        """
        n_splits = split_mask.sum().item()
        ...
        stds3d = torch.exp(self.scales_xyz[split_mask]).repeat(samps, 1)
        means3d = torch.zeros((stds3d.size(0), 3), device=self.xyz.device)
        scaled_samples_xyz = torch.normal(mean=means3d, std=stds3d)
        #quats = self.quats[split_mask] / self.quats[split_mask].norm(dim=-1, keepdim=True)  # normalize them first
        #rots = quat_to_rotmat(quats.repeat(samps, 1))  # how these scales are rotated
        ...
        stds_t = torch.exp(self.scales_t[split_mask]).repeat(samps, 1)
        means_t = torch.zeros((stds_t.size(0), 1), device=self.xyz.device)
        scaled_samples_t = torch.normal(mean=means_t, std=stds_t)
        new_t = scaled_samples_t + self.xyzt[split_mask,3:4].repeat(samps, 1)
        
        new_xyz = self.xyz[split_mask,:3].repeat(samps, 1)
        new_t = new_t
        # step 2, sample new colors
        new_features_dc = self.features_dc[split_mask].repeat(samps, 1, 1)
        new_features_rest = self.features_rest[split_mask].repeat(samps, 1, 1)
        ...
        size_fac = 1.6
        new_scales = torch.log(self.scales[split_mask] / size_fac).repeat(samps, 1)
        new_scale_xyz = self.scales[split_mask,:3].repeat(samps, 1)
    ...
    def get_gaussian_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        return {
            ...
            # for name in ["xyz", "t","scales_xyz","scales_t", "quats1","quats2","accelera", "features_dc", "features_rest", "opacities"]
    ...
    def forward_1(self,camera,background):
        ...
        # camera_to_world_4=torch.zeros(4,4)
        # camera_to_world_4[:3,:4]=camera_to_world
        # camera_to_world_4[3,3]=1
        # camera_to_world=camera_to_world_4
        ...
        # speed_mask = torch.abs(self.scales_t[:,0])>0.5
        # opacity_aftersigmopid[speed_mask]=0
        ...
        # print(delta_t.shape.,speed.shape,means3d.shape)
        dir_pp = (means3d+(delta_t.unsqueeze(-1)*speed)) - camera_center.repeat(N,1)
        ...
        # accelera_masked = self.accelera[temporal_mask,:3]
        ...
        flow_2d = torch.zeros_like(colors_precomp_masked[:,:2])
        rendered_image, radii, depth, flow, alpha = rasterizer(
            means3D=means3D,
            means2D=means2D,
            ts=means4d_masked[:,3].unsqueeze(1),
            speed=speed_masked,
            # accelera=accelera_masked, 
            shs=shs,
        ...
        output = {"rgb": rendered_image.permute(2,1,0), "depth": depth.permute(2,1,0), "accumulation": alpha.permute(2,1,0), "background": background,"flow":flow.permute(2,1,0)}
        self.render = render
        return output
    ...
    def forward_2(self,camera,background):
        ...
        for k, _ in outputs[0].items():
            ...
            elif k in ['render', "depth", "alpha","flow"]:
        ...
        out = {"rgb": self.render['render'].permute(0,3,2,1), 'depth':self.render["depth"].permute(0,3,2,1), "accumulation": self.render['alpha'].permute(0,3,2,1), "background": background,"flow":self.render['flow'].permute(0,3,2,1)}
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        if True:
            ...
            if(len(gt_img.shape)==3):
                rgb=gt_img.unsqueeze(0).permute(0,3,1,2)
                flow=outputs["flow"].unsqueeze(0).permute(0,3,1,2)
            else:
                rgb=gt_img.permute(0,3,1,2)
                flow=outputs["flow"].permute(0,3,1,2)
            device = rgb.device
            dtype = rgb.dtype
            ...
            rgb_edge = sobel_edge(rgb)
            flow_mag = torch.sqrt(flow[:,0:1]**2 + flow[:,1:2]**2 + 1e-6)
            flow_edge = sobel_edge(flow_mag)
            edgeLoss = (flow_edge * (1 - rgb_edge)).mean()    
        ...
        return {
            "main_loss":  self.config.ssim_lambda * (1-simloss)+(1 - self.config.ssim_lambda) * Ll1+self.entropyloss+self.knnloss+(0.05*edgeLoss),
            'scale_reg':scale_reg
        }
